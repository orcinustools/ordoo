...
pub use builder::{BuildOptions, ContainerOptions, ContainerListOptions, ContainerFilter,
                  ...
                  RmContainerOptions, ExecContainerOptions};
...
use rep::{Output, PullInfo, Change, ContainerCreateInfo, ContainerDetails,
          Container as ContainerRep, Event, Exit, History, ImageDetails, Info, SearchResult, Stats,
          Status, Top, Version};
...
impl<'a> Containers<'a> {
    ...
    pub fn create(&'a self, opts: &ContainerOptions) -> Result<ContainerCreateInfo> {
        ...
        let raw = try!(self.docker.post(&path.join("?"), Some((&mut bytes, ContentType::json()))));
